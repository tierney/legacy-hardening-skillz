...
## 1. Discovery Phase
When an agent encounters a repository with a `configure.ac` and multiple `.c`/`.cpp` files with legacy headers, it should activate this skill to ensure behavioral preservation. Performance is measured according to [Evaluation Metrics](evals/eval.yaml).
